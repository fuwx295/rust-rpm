...
impl DependencyFlag {
    pub fn symbol(&self) -> Option<String> {
        let d = self;
        match d {
            _ if d.contains(DependencyFlag::LE) => Some(format!("<=")),
            _ if d.contains(DependencyFlag::GE) => Some(format!(">=")),
            _ if d.contains(DependencyFlag::LESS) => Some(format!("<")),
            _ if d.contains(DependencyFlag::GREATER) => Some(format!(">")),
            _ if d.contains(DependencyFlag::EQUAL) => Some(format!("=")),
            _ => None,
        }
    } 
}
